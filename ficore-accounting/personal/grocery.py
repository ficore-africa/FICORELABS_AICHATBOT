...
from utils import get_mongo_db, requires_role, logger, clean_currency, check_ficore_credit_balance, is_admin
...
def deduct_ficore_credits(db, user_id, amount, action, item_id=None, mongo_session=None):
    try:
        ...
        if current_balance < amount:
            ...
            return False
        session_to_use = mongo_session if mongo_session else client.start_session()
        ...
        if not mongo_session and 'session_to_use' in locals():
...
def index():
    try:
        ...
        logger.info(f"Fetched grocery summary for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify(summary), 200
    except Exception as e:
        logger.error(f"Error fetching grocery summary for user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def manage_lists():
    ...
    try:
        if request.method == 'POST':
            ...
            if current_user.is_authenticated and not is_admin():
                if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                    logger.warning(f"Insufficient Ficore Credits for creating grocery list by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            if current_user.is_authenticated and not is_admin():
                if not deduct_ficore_credits(db, current_user.id, 0.5, 'create_grocery_list', str(result.inserted_id)):
                    ...
                    logger.error(f"Failed to deduct 0.5 Ficore Credits for creating grocery list {result.inserted_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
                    return jsonify({'error': trans('grocery_credit_deduction_failed', default='Failed to deduct Ficore Credits for creating list.')}), 500
            logger.info(f"Created grocery list {result.inserted_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        logger.error(f"Error managing grocery lists for user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def delete_list(list_id):
    ...
    try:
        if not ObjectId.is_valid(list_id):
            logger.error(f"Invalid list_id {list_id}: not a valid ObjectId", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        with db.client.start_session() as mongo_session:
            with mongo_session.start_transaction():
                ...
                if result.deleted_count == 0:
                    logger.error(f"Failed to delete grocery list {list_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def manage_items(list_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if current_user.is_authenticated and not is_admin():
                if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                    logger.warning(f"Insufficient Ficore Credits for adding item to list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            if current_user.is_authenticated and not is_admin():
                if not deduct_ficore_credits(db, current_user.id, 0.5, 'add_grocery_item', str(result.inserted_id)):
                    ...
                    logger.error(f"Failed to deduct 0.5 Ficore Credits for adding item {result.inserted_id} to list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
                    return jsonify({'error': trans('grocery_credit_deduction_failed', default='Failed to deduct Ficore Credits for adding item.')}), 500
            logger.info(f"Added item {result.inserted_id} to list {list_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        if request.method == 'PUT':
            ...
            if current_user.is_authenticated and not is_admin():
                if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                    logger.warning(f"Insufficient Ficore Credits for updating item {item_id} in list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            if updates:
                ...
                if current_user.is_authenticated and not is_admin():
                    if not deduct_ficore_credits(db, current_user.id, 0.5, 'update_grocery_item', item_id):
                        ...
                        logger.error(f"Failed to deduct 0.5 Ficore Credits for updating item {item_id} in list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
                        return jsonify({'error': trans('grocery_credit_deduction_failed', default='Failed to deduct Ficore Credits for updating item.')}), 500
            logger.info(f"Updated item {item_id} in list {list_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        logger.error(f"Error managing items for list {list_id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def share_list(list_id):
    ...
    try:
        ...
        logger.info(f"Shared list {list_id} with {collaborator_email} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'message': trans('grocery_list_shared', default='List shared successfully')}), 200
    except Exception as e:
        logger.error(f"Error sharing list {list_id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def manage_suggestions(list_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            logger.info(f"Added suggestion {result.inserted_id} to list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        logger.error(f"Error managing suggestions for list {list_id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def approve_suggestion(list_id, suggestion_id):
    ...
    try:
        ...
        if current_user.is_authenticated and not is_admin():
            if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                logger.warning(f"Insufficient Ficore Credits for approving suggestion {suggestion_id} in list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        if current_user.is_authenticated and not is_admin():
            if not deduct_ficore_credits(db, current_user.id, 0.5, 'approve_grocery_suggestion', str(result.inserted_id)):
                ...
                logger.error(f"Failed to deduct 0.5 Ficore Credits for approving suggestion {suggestion_id} in list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        logger.info(f"Approved suggestion {suggestion_id} for list {list_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'message': trans('grocery_suggestion_approved', default='Suggestion approved and added to list')}), 200
    except Exception as e:
        logger.error(f"Error approving suggestion {suggestion_id} for list {list_id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def manage_meal_plans():
    ...
    try:
        if request.method == 'POST':
            ...
            if current_user.is_authenticated and not is_admin():
                if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                    logger.warning(f"Insufficient Ficore Credits for creating meal plan by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            if data.get('auto_generate_list'):
                ...
                with db.client.start_session() as mongo_session:
                    with mongo_session.start_transaction():
                        if current_user.is_authenticated and not is_admin():
                            if not deduct_ficore_credits(db, current_user.id, 0.5, 'create_grocery_list_from_meal_plan', str(list_result.inserted_id), mongo_session):
                                logger.error(f"Failed to deduct 0.5 Ficore Credits for creating grocery list {list_result.inserted_id} from meal plan by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
                                raise ValueError(f"Failed to deduct Ficore Credits for creating grocery list {list_result.inserted_id}")
                        for ingredient in meal_plan['ingredients']:
                            ...
                            if current_user.is_authenticated and not is_admin():
                                if not deduct_ficore_credits(db, current_user.id, 0.5, 'add_grocery_item_from_meal_plan', str(item_result.inserted_id), mongo_session):
                                    logger.error(f"Failed to deduct 0.5 Ficore Credits for adding item {item_result.inserted_id} to list {list_result.inserted_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            if current_user.is_authenticated and not is_admin():
                if not deduct_ficore_credits(db, current_user.id, 0.5, 'create_meal_plan', str(result.inserted_id)):
                    ...
                    if data.get('auto_generate_list'):
                        ...
                        db.grocery_items.delete_many({'list_id': str(list_result.inserted_id)})
                    logger.error(f"Failed to deduct 0.5 Ficore Credits for creating meal plan {result.inserted_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            ...
            logger.info(f"Created meal plan {result.inserted_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        logger.error(f"Transaction aborted for creating meal plan or grocery list by user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'error': trans('grocery_credit_deduction_failed', default='Failed to deduct Ficore Credits.')}), 500
    except errors.PyMongoError as e:
        logger.error(f"MongoDB error during meal plan creation for user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'error': trans('grocery_meal_plan_error', default='Error managing meal plans')}), 500
    except Exception as e:
        logger.error(f"Error managing meal plans for user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'error': trans('grocery_meal_plan_error', default='Error managing meal plans')}), 500
...
def add_ingredient(meal_plan_id):
    ...
    try:
        if not ObjectId.is_valid(meal_plan_id):
            logger.error(f"Invalid meal_plan_id {meal_plan_id}: not a valid ObjectId", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            return jsonify({'error': trans('grocery_invalid_meal_plan_id', default='Invalid meal plan ID')}), 400
        meal_plan = db.meal_plans.find_one({'_id': ObjectId(meal_plan_id), 'user_id': str(current_user.id)})
        if not meal_plan:
            return jsonify({'error': trans('grocery_meal_plan_not_found', default='Meal plan not found or not owner')}), 404
        data = request.get_json()
        if not data or not data.get('name') or not data.get('quantity') or not data.get('price'):
            return jsonify({'error': trans('grocery_invalid_ingredient', default='Invalid or missing ingredient data')}), 400
        if current_user.is_authenticated and not is_admin():
            if not check_ficore_credit_balance(required_amount=0.5, user_id=current_user.id):
                logger.warning(f"Insufficient Ficore Credits for adding ingredient to meal plan {meal_plan_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        ...
        if result.modified_count == 0:
            logger.error(f"Failed to add ingredient to meal plan {meal_plan_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
            return jsonify({'error': trans('grocery_ingredient_error', default='Error adding ingredient')}), 500
        if current_user.is_authenticated and not is_admin():
            if not deduct_ficore_credits(db, current_user.id, 0.5, 'add_meal_plan_ingredient', str(meal_plan_id)):
                db.meal_plans.update_one(
                    {'_id': ObjectId(meal_plan_id)},
                    {'$pull': {'ingredients': ingredient}, '$set': {'updated_at': datetime.utcnow()}}
                )
                logger.error(f"Failed to deduct 0.5 Ficore Credits for adding ingredient to meal plan {meal_plan_id} by user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
                return jsonify({'error': trans('grocery_credit_deduction_failed', default='Failed to deduct Ficore Credits for adding ingredient.')}), 500
        logger.info(f"Added ingredient to meal plan {meal_plan_id} for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'message': trans('grocery_ingredient_added', default='Ingredient added to meal plan')}), 200
    except Exception as e:
        logger.error(f"Error adding ingredient to meal plan {meal_plan_id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify({'error': trans('grocery_ingredient_error', default='Error adding ingredient')}), 500
...
def price_history(item_name):
    ...
    try:
        ...
        logger.error(f"Error fetching price history for item {item_name}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
...
def predictive_suggestions():
    try:
        db = get_mongo_db()
        suggestions = get_predictive_suggestions(current_user.id, db)
        logger.info(f"Fetched {len(suggestions)} predictive suggestions for user {current_user.id}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
        return jsonify(suggestions), 200
    except Exception as e:
        logger.error(f"Error fetching predictive suggestions for user {current_user.id}: {str(e)}", extra={'session_id': session.get('sid', 'no-session-id'), 'ip_address': request.remote_addr})
